...
pub async fn get_currency_pairs(less_important_client: Arc<Client>) -> Result<HashMap<String, SymbolInfo>, Box<dyn Error + Send + Sync>> {
 ...
 /*
    ...
    pub async fn get_symbol_details(symbol: &str) -> Option<(f64, f64)> {
        let symbol_data = SYMBOL_DATA.read().await;
        symbol_data.get(symbol).and_then(|details| {
            let adjusted_price = details.adjusted_prices.last().copied()?;
            let num_contracts = details.num_contracts.last().copied()?;
